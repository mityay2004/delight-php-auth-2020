...
class Administration extends UserManager 
{
    ...
    {
        $executed = $this->deleteUsersByColumnValue('email', $email);
    ...
    {
        $userData = $this->getUserDataByUsername(\trim($username),['id']);
    ...
    public function doesUserHaveRole($userId, $role) 
    {
        ...
        $rolesBitmask = $rolesBitmask['roles_mask'];
    ...
    private function deleteUsersByColumnValue($columnName, $columnValue)
    {
        ...
        $res =  $this->db
                ->delete('users', [$columnName => $columnValue]);
        $ec->catchError(new DatabaseError('Error delete user by column ' . $columnName));
        return $res;
    }
    ...
    private function modifyRolesForUserByColumnValue($columnName, $columnValue, callable $modification) 
    {
        ...
        $ec->catchError(new DatabaseError('Error updating user mask'));
    ...
    private function logInAsUserByColumnValue($columnName, $columnValue) 
    {
        ...
        $numberOfMatchingUsers = (\count($users) > 0) ? \count($users) : 0;
