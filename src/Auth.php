            ...
            if (($_SESSION[self::SESSION_FIELD_LAST_RESYNC] + $this->sessionResyncInterval) <= \time()) {
                ...
                if (!empty($authoritativeData)) {
                    ...
                    if ($authoritativeData['force_logout'] > $_SESSION[self::SESSION_FIELD_FORCE_LOGOUT]) {
                        ...
                        $this->logOut();
                    } else { // if the counter that keeps track of forced logouts has remained unchanged
                ...
                } else { // if no data has been found for the user
...
//            );
     ...
     * @param string $password the user's password
     * @param int|null $rememberDuration (optional) the duration in seconds to 
     * keep the user logged in ("remember me"), e.g. `60 * 60 * 24 * 365.25` for one year
     * @param callable|null $onBeforeSuccess (optional) a function that receives the user's ID as its single parameter and is executed before successful authentication; must return `true` to proceed or `false` to cancel
    ...
    public function reconfirmPassword($password)
    {
        if ($this->isLoggedIn()) {
            $this->throttle(
                    ['reconfirmPassword', $this->getIpAddress()],
                    3,
                    (60 * 60),
                    4,
                    true
            ...
            $expectedHash = $expectedHash['password'];
    ...
    private function createRememberDirective($userId, $duration)
    {
        ...
        $this->db->insert('users_remembered',
                [
                    'user' => $userId,
                    'selector' => $selector,
                    'token' => $tokenHashed,
        ...
        $ec->catchError(new DatabaseError('Error creating remembered directive'));
    ...
    public function confirmEmail($selector, $token)
    {
        ...
        $confirmationData = $this->db
            ->query(
                'SELECT a.id, a.user_id, a.email AS new_email, a.token, a.expires, b.email old_email
                    FROM users_confirmations a
            ...
            if (\password_verify($token, $confirmationData['token'])) {
                if ($confirmationData['expires'] >= \time()) {
                    ...
                    $this->db
                            ...
                            ->delete('users_resets');
                    $ec->catchError(new DatabaseError('Error deleting resetted user'));
                    ...
                    $ec->catchError(new DatabaseError('Error delete users confirmation'));
    ...
    public function changeEmail($newEmail, callable $callback)
    {
        if ($this->isLoggedIn()) {
            ...
            $existingUsersWithNewEmail = $this->db
                ->query(
                    'SELECT COUNT(*) cnt 
                        FROM users
                        WHERE email = ?',
            ...
            if (is_null($existingUsersWithNewEmail) || (int)$existingUsersWithNewEmail['cnt'] !== 0) {
    ...
    {
        $latestAttempt = $this->db
            ->query(
                'SELECT user_id, email FROM users_confirmations
                    WHERE ' . $columnName . ' = ?
                    ORDER BY id DESC
                    LIMIT 1 OFFSET 0',
                [ $columnValue ]
        ...
        $this->throttle(
                ['resendConfirmation', 'userId', $latestAttempt['user_id']],
                1,
                (60 * 60 * 6)
        );
        $this->throttle(
                ['resendConfirmation', $this->getIpAddress()],
                4,
                (60 * 60 * 24 * 7),
                2
        );
    ...
    public function forgotPassword($email, callable $callback, $requestExpiresAfter = null, $maxOpenRequests = null)
    {
        ...
        if ($openRequests < $maxOpenRequests) {
            $this->throttle(
                    ['requestPasswordReset', $this->getIpAddress()],
                    4,
                    (60 * 60 * 24 * 7),
                    2
            );
            $this->throttle(
                    ['requestPasswordReset', 'user', $userData['id']],
                    4,
                    (60 * 60 * 24 * 7),
                    2
            );
            ...
            callable $onBeforeSuccess = null) 
    {
        ...
        $this->throttle(
                ['attemptToLogin', $this->getIpAddress()],
                4,
                (60 * 60),
                5,
                true
        ...
        } else { // if neither an email address nor a username has been provided
    ...
    private function getOpenPasswordResetRequests($userId)
    {
        ...
        if (!is_null($requests) and !empty($requests['cnt'])) {
                return $requests['cnt'];
        }
        else {
                return 0;
    ...
    private function createPasswordResetRequest($userId, $expiresAfter, callable $callback)
    {
        ...
        $ec->catchError(new DatabaseError('Error creating password reset'));
    ...
    {
        $this->throttle(
                ['resetPassword', $this->getIpAddress()],
                5,
                (60 * 60),
                10
        );
        ...
        $resetData = $this->db
            ->query(
                'SELECT a.id, a.user, a.token, a.expires, b.email, b.resettable
                    FROM users_resets a 
                    JOIN users b ON b.id = a.user
                    WHERE a.selector = ?',
                [ $selector ]
            )
            ->row_array();
                ...
                if (\password_verify($token, $resetData['token'])) {
                    if ($resetData['expires'] >= \time()) {
                        ...
                        $this->db
                            ...
                            ->delete('users_resets');
                        $ec->catchError(new DatabaseError('Error clean resetting user'));
    ...
    public function setPasswordResetEnabled($enabled)
    {
        ...
        if ($this->isLoggedIn()) {
            ...
            $ec->catchError(new DatabaseError('Error make password resettable'));
        } else {
            throw new NotLoggedInException();
        }
    }
            ...
            $cost = null)
    {
        ...
        $bucket = $this->db
            ->query(
                'SELECT tokens, replenished_at 
                    FROM users_throttling
                    WHERE bucket = ?',
                [ $key ]
            )
            ->row_array();
        ...
        if (!$simulated) {
            ...
            if ($affected === 0) {
                $bucket['bucket'] = $key;
                $this->db
                        ->insert('users_throttling', $bucket);
            }
        }
    ...
    private function getRememberDirectiveExpiry()
    {
        ...
        if ($this->isLoggedIn()) {
            ...
            if (isset($existingSelector)) {
                ...
                $existingExpiry = $this->db
                    ->query(
                        'SELECT expires
                            FROM users_remembered
                            WHERE selector = ? AND user = ?',
                        [$existingSelector, $this->getUserId()]
                    )
                    ->row_array();
                
                if (is_null($existingExpiry)) {
                    return null;
                }
                $existingExpiry = $existingExpiry['expires'];
